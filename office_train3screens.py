...
#    print(userindex.keys())
    ...
    for filename in os.listdir("./mytestdata/office3screens"):
...
#                continue
            ...
            modified_splitindex = splitindex
            pred_index = allindex[int(len(allindex)*ratio):]
            __data = pd.read_csv('./mytestdata/office3screens/'+filename)
            for ridx,row in __data['index'].items():
                if row >= splitindex:
                    modified_splitindex = ridx
                    break
            #print(allindex)
            #print(splitindex)
            #print(pred_index)
            ...
            summary_data = SummaryDataModule(tokenizer, './mytestdata/office3screens/'+filename,
                                             batch_size = 14, num_examples = modified_splitindex)
            ...
            ckpt_dir = './checkpoint_files_2/office_'+user+'_3screens'
            ...
            pred_df = pd.read_csv('./mytestdata/office3screens/'+filename)[splitindex:]
...
#                print(rankings(row['source'],out_dict))
        ...
        with open('./mytestdata/office_suggest3screen.json', 'w') as outfile:
