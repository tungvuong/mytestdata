...
def main():
    ...
    for filename in os.listdir("./mytestdata/5screens"):
        if filename.endswith(".csv"):
            ...
            summary_data = SummaryDataModule(tokenizer, './mytestdata/5screens/'+filename,
            ...
            ckpt_dir = './checkpoint_files_2/'+user+'_5screens'
            ...
            pred_df = pd.read_csv('./mytestdata/5screens/'+filename)[splitindex:]
            for index, row in pred_df.iterrows():
                if (index+4 not in pred_index):
                    continue
                print()
                print(index+4)
...
#                print(rankings(row['source'],out_dict))
                suggestions[user].append([row['title'],row['target'],pred_target,row['source'],index+4])
        with open('./mytestdata/suggest5screen.json', 'w') as outfile:
