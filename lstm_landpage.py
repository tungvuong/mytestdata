...
def main():
    ...
    for filename in os.listdir("./mytestdata/land_3screens"):
        if filename.endswith(".csv"):
            ...
            print(len(train))
            print(len(test))
            if os.path.exists('./mytestdata/lstm_data/saved/land_'+user):
                continue
            rows = []
            for row in train['source']:
                rows.append(row.split('</s>')[0])
            with open('./mytestdata/lstm_data/land_'+user+'.txt', 'w') as f:
                for line in rows:
                    f.write(f"{line[:500]}\n")
            ...
            for i in range(NUM_TRAIN_BATCHES):
                ...
                if (i % 100) == 0:
                    ...
                    saver.save(sess, './mytestdata/lstm_data/saved/land_'+user+'/model.ckpt')
            sess.close()
            #break
