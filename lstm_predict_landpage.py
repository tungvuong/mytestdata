    ...
    for filename in os.listdir("./mytestdata/land_3screens"):
        if filename.endswith(".csv"):
            ...
            if user in ['D43D7EC3E0C2']:
                ratio = 0.85
            print('--------------',user,ratio,'----------')
            data = pd.read_csv('./mytestdata/land_3screens/'+filename)
            ...
            saver.restore(sess, './mytestdata/lstm_data/saved/land_'+user+'/model.ckpt')
            
            pred_df = pd.read_csv('./mytestdata/land_3screens/'+filename)[len(train):]
            ...
            with open('./mytestdata/lstm3screen_land.json', 'w') as outfile:
