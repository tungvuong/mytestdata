...
def main():
    ...
    for filename in os.listdir("./mytestdata/7screens"):
        if filename.endswith(".csv"):
            ...
            summary_data = SummaryDataModule(tokenizer, './mytestdata/7screens/'+filename,
            ...
            ckpt_dir = './checkpoint_files_2/'+user+'_7screens'
            ...
            pred_df = pd.read_csv('./mytestdata/7screens/'+filename)[splitindex:]
            for index, row in pred_df.iterrows():
                if (index+6 not in pred_index):
                    continue
                print()
                print(index+6)
...
#                print(rankings(row['source'],out_dict))
                suggestions[user].append([row['title'],row['target'],pred_target,row['source'],index+6])
        with open('./mytestdata/suggest7screen.json', 'w') as outfile:
