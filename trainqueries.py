...
summary_data = SummaryDataModule(tokenizer, './queries.csv',
                                 batch_size = 2, num_examples = 64)
...
def generate_lyrics(seed_line, num_lines, model_, noise_percent = 0.25, multiple_lines = False, max_line_history = 3):
  ...
  prompt_line_tokens = tokenizer(noise_sentence(seed_line, 0.2), max_length = 232, return_tensors = "pt", truncation = True)
...
#      prompt_line = lyrics[i]
    #print(i,prompt_line)
#    prompt_line_tokens = tokenizer(noise_sentence(prompt_line, noise_percent), max_length = 232, return_tensors = "pt", truncation = True)

  return lyrics



pred_df = pd.read_csv('queries.csv')[64:]
