...
def main():
    ...
    for filename in os.listdir("./mytestdata/pickrandom3s"):
        if filename.endswith(".csv"):
            ...
            summary_data = SummaryDataModule(tokenizer, './mytestdata/random3s/'+filename,
            ...
            ckpt_dir = './checkpoint_files_2/'+user+'_pickrandom3s'
            ...
            pred_df = pd.read_csv('./mytestdata/pickrandom3s/'+filename)[splitindex:]
            for index, row in pred_df.iterrows():
                if (index+5 not in pred_index):
                    continue
                print()
                print(index+5)
...
#                print(rankings(row['source'],out_dict))
                suggestions[user].append([row['title'],row['target'],pred_target,row['source'],index+5])
        with open('./mytestdata/pickrandom3s.json', 'w') as outfile:
